# -*- coding: utf-8 -*-
"""
Tests for the modular local search parser.
"""
...
class PriceExtractionTests(unittest.TestCase):
    ...
    def test_extract_price_zir_gives_max_price(self):
        ...
        min_p, max_p, remaining = extract_price(
            "کفش سفید زیر 3 میلیون تهران"
        )
...
class LocalQueryParserTests(unittest.TestCase):
    ...
    def test_full_example_query_from_the_spec(self):
        ...
        self.assertEqual(sq.gender, "مردانه")
    ...
    def test_gender_word_variants_normalize_to_canonical_form(self):
        self.assertEqual(
            self.parser.parse("پیراهن زنونه").gender,
            "زنانه",
        )
        self.assertEqual(
            self.parser.parse("پیراهن زنانه").gender,
            "زنانه",
        )
        self.assertEqual(
            self.parser.parse("کفش بچگانه").gender,
            "بچگانه",
        )
    ...
    def test_simplified_drops_price_and_city_but_keeps_keyword_and_category(self):
        sq = self.parser.parse(
            "کفش سفید مردونه زیر 3 میلیون تهران"
        )
...
class ReferralDeepLinkTests(unittest.TestCase):
    ...
    def test_rejects_garbage_payload(self):
        ...
        self.assertIsNone(
            regex.match("not_a_shop_link")
        )
        self.assertIsNone(
            regex.match("shop_")
        )
        self.assertIsNone(
            regex.match("shop_abc")
        )
